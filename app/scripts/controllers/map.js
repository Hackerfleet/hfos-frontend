...
angular.module('hfosFrontendApp')
    .controller('MapCtrl', function ($scope, leafletData, navdata, socket, user, createDialog, ObjectProxy, Detector, MapViewService,
        ...
        $scope.mapviewuuid = user.clientconfig().mapviewuuid;
        console.log('[MAP] Configured Mapview:', $scope.mapviewuuid);
        $scope.mapview = {};
        ...
        var updateMapview = function (objuuid) {
            if (objuuid === $scope.mapviewuuid) {
                ...
                $scope.mapview = ObjectProxy.obj[objuuid];
            }
        };

        $scope.$on('OP.Get', function (event, objuuid) {
            updateMapview(objuuid);
        });

        $scope.$on('OP.Update', function (event, objuuid) {
            updateMapview(objuuid)
            syncFromMapview();
        });

        var syncToMapview = function () {
            ...
            if ($scope.mapviewuuid !== '') {
                ...
                if ($scope.sync) {
                    ObjectProxy.put("mapview", $scope.mapview);
                }
        ...
        var syncFromMapview = function () {
            if ($scope.mapviewuuid !== '') {
                $scope.center = $scope.mapview.coords;
                console.log('Sync from MV: ', $scope.center);
            }
        };


        var subscribe = function () {
            ObjectProxy.subscribe($scope.mapviewuuid);
        };

        var unsubscribe = function () {
            ObjectProxy.unsubscribe($scope.mapviewuuid);
        };
        ...
        var getMapview = function () {
            ...
            console.log('[MAP] Mapview UUID: ', $scope.mapviewuuid);

            if ($scope.mapviewuuid === '') {
                console.log('[MAP] Picking user default mapview', user.profile());
                $scope.mapviewuuid = user.profile().settings.mapviewuuid;
            }
            console.log('[MAP] Mapview UUID: ', $scope.mapviewuuid);

            if ($scope.mapviewuuid !== '') {
                ObjectProxy.get('mapview', $scope.mapviewuuid);
            }
        };
...
//              success: {label: 'Select', fn: subscribe},
        ...
        $scope.$on('Clientconfig.Update', function (event) {
        ...
        var handleEvent = function (event) {
            if (user.signedin()) {
                ...
                if (event.name === 'leafletDirectiveMap.moveend') {
                    ...
                    if (subscriptionuuid !== '') {
                        ObjectProxy.get('mapview', subscriptionuuid);
                    }
                }
            }
        };
    ...
    }).controller('MapViewSelectCtrl', function ($scope, mapview, mapviews, user) {
    $scope.mapview = mapview;
